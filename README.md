...
基于 GeWe 的单组织私有化微信机器人管理系统。GeWe 负责连接微信，本系统负责账号、Webhook、目录持久化、可靠发送、插件、群/成员级权限和审计；MaiBot 通过独立 Connector 接入，受限 Tool Broker 统一承接受控工具调用，Task Agent 负责后续多步骤办事能力。
...
- MaiBot Connector 已完成受管 WebSocket、ACK/重连/TTL/fencing、回复与主动消息权限链路；受限 Tool Broker、Tool Call ledger 和 MaiBot Tool protocol adapter 已接入本地运行链路并有契约测试，真实 MaiBot 协议联调仍未完成。
- Task Agent 已完成持久数据模型、Run 状态机、幂等、等待用户回答、受 RBAC 保护的管理 API 和后台工作台；模型 Worker、Task Agent 调用 Tool Broker 的入口、通用审批、预算及微信/MaiBot 任务入口仍属后续阶段。
...
- Tool Bridge 当前只允许通过平台签发来源上下文请求 Connector allowlist 中的低风险 `READ_ONLY` Tool；Broker 会在执行前后校验 ACL、grant、Schema、幂等键和 activation epoch，并记录调用账本。Task Agent 尚未接入该 Broker，`AUTONOMOUS` 调用在 MVP 中拒绝。
...
- 真实 GeWe、MaiBot、模型 Provider、公网回调和生产 PostgreSQL 尚未完成联合验收；受限 Tool Bridge 目前只有本地或 Mock 契约证据，对应能力不能标注为真实生产可用。
